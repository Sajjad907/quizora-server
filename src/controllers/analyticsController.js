...
exports.exportLeads = asyncHandler(async (req, res) => {
  ...
  if (!leads || leads.length === 0) {
    return res.status(404).json({ status: "fail", message: "No leads found to export." });
  }

  // Generate CSV manually
  const headers = ["Lead ID", "First Name", "Last Name", "Email", "Source Quiz", "Captured Date"];
  const rows = leads.map(lead => [
    lead._id.toString(),
    `"${(lead.firstName || '').replace(/"/g, '""')}"`,
    `"${(lead.lastName || '').replace(/"/g, '""')}"`,
    `"${(lead.email || '').replace(/"/g, '""')}"`,
    `"${(lead.quizId?.title || 'Unknown').replace(/"/g, '""')}"`,
